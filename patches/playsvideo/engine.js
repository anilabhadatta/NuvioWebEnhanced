...
export class PlaysVideoEngine extends EventTarget {
    ...
    loadFile(file, opts) {
        ...
        this._audioTrackIndex = opts?.audioTrackIndex ?? null;
        this._pendingFileType = file.type || null;
        this._blobUrl = URL.createObjectURL(file);
        this._keyframeIndex = opts?.keyframeIndex ?? null;
        this.createWorker();
        this.worker.postMessage({ type: 'open', file, audioTrackIndex: this._audioTrackIndex });
    ...
    loadUrl(url, opts) {
        ...
        this.createWorker();
        this.worker.postMessage({ type: 'open-url', url, audioTrackIndex: this._audioTrackIndex });
        mlog(`open url=${url}`);
    }
    ...
    reset(detail) {
        ...
        this._audioTracks = [];
        this._selectedAudioTrackIndex = -1;
    ...
    selectAudioTrack(index) {
        if (this._selectedAudioTrackIndex === index) {
            return;
        }
        if (this._passthrough) {
            const nativeAudioTracks = this.video.audioTracks;
            if (nativeAudioTracks && nativeAudioTracks[index]) {
                for (let i = 0; i < nativeAudioTracks.length; i++) {
                    nativeAudioTracks[i].enabled = (i === index);
                }
        ...
        if (this._currentUrl) {
            this.loadUrl(this._currentUrl, {
                keyframeIndex: this._keyframeIndex,
            ...
            this.loadSource(this._currentSource, {
                keyframeIndex: this._keyframeIndex,
                audioTrackIndex: index,
            });
        }
    ...
    handleWorkerMessage(event) {
        ...
        if (msg.type === 'probed') {
            ...
            this._selectedAudioTrackIndex = msg.selectedAudioTrackIndex ?? -1;
    ...
    async startSourcePipeline(source) {
        try {
            ...
            if (selectedOption.mode !== 'hls') {
                ...
                this._selectedAudioTrackIndex = demux.selectedAudioTrackIndex ?? -1;
