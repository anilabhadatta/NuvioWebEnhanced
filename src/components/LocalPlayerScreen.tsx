...
const MoviPlayerWrapper = React.memo(({ resolvedSrc, onInit }: { resolvedSrc: string | File, onInit: (p: any) => void }) => {
  ...
  useEffect(() => {
    ...
    if (resolvedSrc) {
      ...
      if (resolvedSrc instanceof File) {
        if (player.src !== resolvedSrc) {
          player.src = resolvedSrc;
          player.style.display = "block";
        }
      } else {
        if (player.getAttribute("src") !== resolvedSrc) {
...
const getLocalTestingId = (src: string | File): string => {
  if (src instanceof File) {
    return `local_file_${src.name}_${src.size}`;
  }
  // Sanitize the URL to make a safe storage ID
  return `local_url_${src.replace(/[^a-zA-Z0-9]/g, "_").slice(-100)}`;
};
...
export default function LocalPlayerScreen() {
  ...
  const [localMovieId, setLocalMovieId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (!targetMatch && prefs.addonSubtitleStartup === "Always on") {
      targetMatch = addonSubtitles[0];
    }

    if (targetMatch && loadExternalSubtitleRef.current) {
      console.log(`[MoviPlayer] Auto-selecting addon subtitle: ${targetMatch.name}`);
      autoSelectedAddonSubRef.current = true;
      loadExternalSubtitleRef.current(targetMatch.id, targetMatch.url, targetMatch.name);
    }
  ...
  const movieId = rawMovieId ? rawMovieId.replace(/^tmdb:/, "") : localMovieId;
  ...
  onStateChangeRef.current = (e: any) => {
    ...
    const isMatching = resolvedSrc instanceof File ? video.src === resolvedSrc : video.getAttribute("src") === resolvedSrc;
  ...
  onTracksChangeRef.current = (e: any) => {
    ...
    if (!subPrefAppliedRef.current) {
      if (subtitle?.length > 0) {
        ...
        // No built-in tracks at all — unlock so addon subtitles can take over
        console.log("[MoviPlayer] No built-in subtitle tracks in stream — unlocking for addon subtitles.");
        didUpdateSubPrefs = true;
        setSelectedSub(-1);
      }
      subPrefAppliedRef.current = true;
...
EventDump: ${JSON.stringify(collected)}`;
  ...
  const openInExternalPlayer = useCallback((id: string) => {
    ...
    if (!target || !resolvedSrc || typeof resolvedSrc !== "string") return;
    try {
  ...
  const copyStreamUrl = useCallback(async () => {
    if (!resolvedSrc || typeof resolvedSrc !== "string") return;
    try {
  ...
  // Register this function in the ref so the addon-subtitle useEffect (defined earlier) can call it
  loadExternalSubtitleRef.current = loadExternalSubtitle;
  ...
  return (
    ...
    >
      ...
      {/* States */}
      {!resolvedSrc && (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-white/50 z-10 pointer-events-auto">
          {isLocalTesting ? (
            <div className="flex flex-col items-center gap-4 max-w-md text-center px-6">
              <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center border border-white/10 mb-2">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-8 h-8 text-white/60">
              ...
              <button
                ...
                className="mt-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl shadow-lg transition-all text-sm hover:scale-[1.02] active:scale-[0.98]"
              >
                Add Link / Local File
              </button>
            </div>
      ...
      <div className={`absolute inset-0 transition-opacity duration-300 pointer-events-none z-20 flex flex-col justify-between ${showControls ? "opacity-100" : "opacity-0"}`}>
        ...
        <div className="bg-gradient-to-b from-black/80 to-transparent p-6 flex items-center pointer-events-auto">
          ...
          <button
            onClick={() => {
              if (videoRef.current) {
                const event = new KeyboardEvent('keydown', { key: 'i', bubbles: true, composed: true });
        ...
        <div className="bg-gradient-to-t from-black/90 via-black/50 to-transparent p-6 pointer-events-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              {/* Add Link (Local Testing Mode) */}
              {isLocalTesting && (
                <button
                  onClick={() => setShowAddLinkModal(true)}
                  className="flex items-center gap-1.5 px-3 h-10 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-[11px] font-semibold transition-all hover:scale-[1.02] active:scale-[0.98] select-none cursor-pointer"
                  title="Load URL stream or local file"
                >
              ...
              <button
                ...
                disabled={!resolvedSrc || (resolvedSrc instanceof File)}
                className="w-10 h-10 flex items-center justify-center text-white/80 hover:text-white transition-colors bg-white/10 hover:bg-white/20 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
                title={resolvedSrc instanceof File ? "External player unsupported for local files" : "Open in external player"}
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-5 h-5">
          ...
          >
            ...
            <form
              onSubmit={(e) => {
                ...
                const urlVal = (formData.get("streamUrl") as string || "").trim();
                const fileInput = e.currentTarget.querySelector('input[type="file"]') as HTMLInputElement;
                const fileVal = fileInput?.files?.[0];

                if (fileVal) {
                  setResolvedSrc(fileVal);
                  setLocalMovieId(getLocalTestingId(fileVal));
                  setLocalTitle(fileVal.name);
                  setShowAddLinkModal(false);
                } else if (urlVal) {
                  setResolvedSrc(urlVal);
                  setLocalMovieId(getLocalTestingId(urlVal));
                  setLocalTitle(urlVal.split("/").pop()?.split("?")[0] || "Custom Stream");
                  setShowAddLinkModal(false);
                }
              }}
              className="space-y-5"
            >
              <div>
                <label className="block text-white/60 text-xs font-semibold uppercase tracking-wider mb-2">Video Stream URL</label>
                <input
                  name="streamUrl"
                  type="url"
                  placeholder="Paste TorBox, direct MP4/MKV/HLS URL..."
              ...
              <div className="relative flex py-2 items-center">
                ...
                <span className="flex-shrink mx-4 text-white/30 text-xs font-bold uppercase tracking-widest">OR</span>
                <div className="flex-grow border-t border-white/5"></div>
              </div>

              <div>
                <label className="block text-white/60 text-xs font-semibold uppercase tracking-wider mb-2">Local File</label>
                <div className="relative group cursor-pointer bg-white/5 hover:bg-white/10 border border-dashed border-white/10 hover:border-white/30 rounded-xl p-6 text-center transition-all">
                  <input
                    type="file"
                    accept="video/*"
                    onChange={(e) => {
                      ...
                      if (file) {
                        setResolvedSrc(file);
                        setLocalMovieId(getLocalTestingId(file));
                  ...
                  />
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-8 h-8 text-white/40 mx-auto mb-2 group-hover:scale-105 transition-transform">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 16.5V9.75m0 0l3 3m-3-3l-3 3M6.75 19.5a4.5 4.5 0 01-1.41-8.775 5.25 5.25 0 0110.233-2.33 3 3 0 013.758 3.848A3.752 3.752 0 0118 19.5H6.75z" />
                  </svg>
                  <p className="text-white/70 text-sm font-semibold">Choose video file</p>
                  <p className="text-white/40 text-xs mt-1">Supports MP4, MKV, WebM, etc.</p>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-3">
                <button
                  type="button"
                  onClick={() => setShowAddLinkModal(false)}
                  className="flex-1 py-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl text-white font-semibold text-sm transition-all text-center"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl text-white font-semibold text-sm shadow-lg transition-all text-center"
                >
                  Play
                </button>
              </div>
