...
// vlc-player's built-in color palette (matches VlcElement.SUBTITLE_COLOR_PALETTE)
...
interface SubtitleStyle {
  ...
  bgColor: string;  // hex or "auto" (auto = vlc-player picks contrast color)
...
// Push style onto the live vlc-player element via its native attribute API.
// vlc-player handles persistence to localStorage["movi.subtitleSettings"] internally.
//
// NOTE: Each setAttribute call triggers vlc-player's attributeChangedCallback which
...
function applySubtitleStyleToPlayer(player: any, style: SubtitleStyle) {
  ...
  // Defer so we run AFTER vlc-player's synchronous applySubtitleSettings() resets the var
  setTimeout(() => {
    ...
    // Force vlc-player to ALWAYS apply the background, even on non-VTT (SRT/embedded) tracks.
    // By default vlc-player only applies it to .movi-subtitle-format-vtt.
...
// ---------------------------------------------------------------------------
// vlc-player wraps Shaka Player internally. By default Shaka uses a software
...
function configureShakaPerformance(moviElement: any) {
  try {
    // vlc-player exposes its internal Shaka instance as .player
...
function ensureVlcPlayerLoaded(): Promise<void> {
  ...
  VlcPlayerLoadPromise = new Promise<void>((resolve) => {
    ...
    customElements.define("vlc-player", VlcPlayer);
    resolve();
  });
  return VlcPlayerLoadPromise;
}

const VlcPlayerWrapper = React.memo(({ resolvedSrc, onInit }: { resolvedSrc: string, onInit: (p: any) => void }) => {
  const wrapperRef = useRef<HTMLDivElement>(null);
  const playerRef = useRef<any>(null);
  const [vlcReady, setMoviReady] = useState(false);
  ...
  useEffect(() => {
    ...
    let player = wrapperRef.current.querySelector("vlc-player") as any;
    if (!player) {
      wrapperRef.current.innerHTML = `<vlc-player class="w-full h-full object-contain" playsinline="true"></vlc-player>`;
      player = wrapperRef.current.querySelector("vlc-player") as any;
      playerRef.current = player;
      onInit(player);
    }

    ensureVlcPlayerLoaded().then(() => {
    ...
    return () => {
      ...
      if (player) {
        try {
          ...
          console.error("Error during vlc-player cleanup", e);
        }
      }
    };
  }, [onInit]);
  ...
  // This guarantees vlc-player's attributeChangedCallback processes the src
  // and starts its internal Shaka player.
  useEffect(() => {
    const player = playerRef.current ?? wrapperRef.current?.querySelector("vlc-player");
    if (!player || !vlcReady) return;
...
export default function VlcPlayerScreen() {
  ...
  // Subtitle style — backed by vlc-player's native attribute API.
  // We read vlc-player's own localStorage key on mount so the UI
  ...
  const updateSubtitleStyle = (newStyle: SubtitleStyle) => {
    ...
    // Persist bgColor separately — vlc-player's own save would overwrite our field
  ...
  // reads subtitleDelay state) and vlc-player's own renderer for built-in/muxed
  ...
  // Uses vlc-player's native setAttribute API — the element translates these into
  ...
  // vlc-player module loading is now handled inside VlcPlayerWrapper
  ...
  useEffect(() => {
    ...
    async function resolveUrl() {
      ...
      //    CDN URL. The CDN itself is CORS-clean so vlc-player plays directly.
...
EventDump: ${JSON.stringify(collected)}`;
  ...
  const handleSubtitleChange = (id: number) => {
    ...
    if (moviEl) {
      // Clean any previously injected sidecar <track> children from <vlc-player>
